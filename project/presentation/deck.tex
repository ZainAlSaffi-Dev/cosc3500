...
% The four null levels share one frame in the recording. Their individual
% frames, with the code screenshots, are in the backup section.

\begin{frame}{The levels that measured nothing}{Levels 0, 1, 3 and 4}
  \sigtable{lll}{\textbf{Level} & \textbf{Adds} & \textbf{Why nothing moved}}{%
    0, harness control & nothing at all & a function pointer picked once is free \\
    1, hoisting & invariants moved out of the loops & GCC at -O2 had already hoisted them \\
    3, traversal order & inner loop in memory layout order & the baseline already walks rows \\
    4, loop splitting & a separate loop for the $v=0$ row & the baseline already splits it \\}
  \sigcallout{A null is still a measurement. It says the technique was
    already applied, by the compiler or by the baseline's design. The
    per-level code is in the backup slides.}
\end{frame}
