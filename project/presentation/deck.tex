  ...
  \begin{sigcolumns}
    \begin{sigcolumn}
      \begin{itemize}
        ...
        \item The \sigemph{spacing} is the gap between neighbouring
          values, about \$10 between share prices here. Finer spacing
          tracks the true curve better and multiplies the number of
          cells.
        \item At expiry every cell equals the payoff, so the solver
          starts there and steps \sigemph{backwards} to today.
...
\begin{frame}{How the optimisation was structured}{Seven solvers, one technique per level}
  \begin{itemize}
...
% Levels 0, 1 and 2 keep their own frames. Levels 3 and 4 share one frame
% in the recording, and their individual frames are in the backup section.
...
\begin{frame}{Levels 3 and 4}{The baseline already did both, so throughput did not change}
  \setlength{\sigslotheight}{0.5\textheight}
  \begin{sigcolumns}
    \begin{sigcolumn}
      \sigfigure{../results/code_shots/shot_L3_order.png}{Level 3,
        traversal order. The loops walk each row in the order it sits in
        memory, so every 64-byte cache line is fully used before it is
        evicted.}
    \end{sigcolumn}
    \begin{sigcolumn}
      \sigfigure{../results/code_shots/shot_L4_split.png}{Level 4, loop
        splitting. The $v=0$ row gets its own loop instead of an if in
        every cell.}
    \end{sigcolumn}
  \end{sigcolumns}
  \sigcallout{Throughput did not change at either level, because the
    baseline already walked rows in memory order and already split the
    $v=0$ row out. What the walking order is worth, the deliberately
    swapped version on the ladder slide shows.}
\end{frame}
...
\begin{frame}{Level 5, induction variables}{Row pointers instead of index arithmetic}
  ...
  \sigrungnotes{Each row is walked with pointers that advance one step per
    ...
    neighbour.}{The intended win is about ten fewer integer additions per
    cell, if that work is not already hidden.}
  \sigcallout{A small gain, the only one after level 2. Neighbour offsets
    are computed inside the load instructions at no extra cost, and address
    adds run on separate execution ports, so most of this work was already
    hidden behind the floating point.}
\end{frame}
...
\begin{frame}{The ladder, measured}{Only level 2 changed throughput}
  ...
  \sigcallout{Swapping the loops on purpose makes identical arithmetic
    \sigemph{2.1x slower}, so the traversal order the baseline already
    had was doing real work. An automated test holds every level to the
    baseline answer, within 15 units in the last decimal place.}
\end{frame}
...
\begin{frame}[plain]
  ...
  {\Large\color{sigink}\bfseries A 2.56x serial speedup, validated at
    every level}
  \par\vspace{0.6em}
  {\color{sigslate} COSC3500 Milestone 1, Zain Al-Saffi}
  \vfill
\end{frame}
...
\begin{frame}{Convergence}{Halve the grid spacing and the error should fall four times over}
  \begin{sigcolumns}
    \begin{sigcolumn}
      \begin{itemize}
        \item The same option is priced on a sequence of grids, each with
          half the spacing of the last, and each price is compared with
          the Fourier reference.
        \item The error falls about \sigemph{four times over per halving}.
          That is the second-order accuracy the stencil is built to have,
          so the slope is itself a check on the code.
        \item Below about \$0.014 refinement stops helping. The grid has
          to end somewhere, and the reference carries its own small
          error, so those two set the floor.
      \end{itemize}
    \end{sigcolumn}
    \begin{sigcolumn}
      \sigfigure{../results/convergence.png}{Error against the Fourier
        price as the spacing halves.}
    \end{sigcolumn}
  \end{sigcolumns}
\end{frame}
