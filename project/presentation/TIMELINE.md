...
| 1:45 | 0:15 | From stochastic model to deterministic equation. There is no random number generator in the program. | a text slide | small webcam |
| 2:00 | 0:10 | The derivative notation. Subscripts denote partial derivatives. | a table slide | small webcam |
| 2:10 | 0:15 | Why the noise terms become second derivatives. | a text slide | small webcam |
| 2:25 | 0:10 | The equation term by term. Each part of the model contributes one term. | a table slide | small webcam |
| 2:35 | 0:20 | The full equation, known numbers separated from the one unknown. | a text slide | small webcam |
| 2:55 | 0:30 | The grid and its spacing, filled in backwards from expiry. | `results/demo_timevalue.mp4` again, this time explained | small webcam |
| 3:25 | 0:20 | Validation begins. Switch the model off and it becomes one we can check exactly. | `results/bs_collapse.mp4` | small webcam |
| 3:45 | 0:15 | Three methods that share no code agree on one number. | a table slide with the three prices | small webcam |
| 4:00 | 0:15 | The convergence study. Halving the spacing cuts the error four times over, then the floor. | `results/convergence.png` | small webcam |
| 4:15 | 0:15 | Why the optimisation study is a ladder rather than a before and after. | a text slide | small webcam |
| 4:30 | 0:15 | Level 0, the harness control. It confirms the harness costs nothing. | `results/code_shots/shot_L0_anchor.png` | small webcam |
| 4:45 | 0:15 | Level 1, hoisting. The compiler had already done it. | `results/code_shots/shot_L1_hoisting.png` | small webcam |
| 5:00 | 0:25 | Level 2, strength reduction. The change the compiler is not allowed to make. | `results/code_shots/shot_L2_strength_reduction.png` | small webcam |
| 5:25 | 0:15 | Level 3, traversal order. Null by construction. | `results/code_shots/shot_L3_order.png` | small webcam |
