...
| 2:45 | 0:30 | The grid and its spacing, filled in backwards from expiry. | `results/demo_timevalue.mp4` again, this time explained | small webcam |
| 3:15 | 0:20 | Validation begins. Switch the model off and it becomes one we can check exactly. | `results/bs_collapse.mp4` | small webcam |
| 3:35 | 0:15 | Three methods that share no code agree on one number. | a table slide with the three prices | small webcam |
| 3:50 | 0:15 | The convergence study. Halving the spacing cuts the error four times over, then the floor. | `results/convergence.png` | small webcam |
