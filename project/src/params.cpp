...
// File-local helpers. 'static' here means visible only inside this .cpp —
...
static double parse_double(const std::string& value, const std::string& where) {
    ...
    // 'used' is how many characters stod consumed — reject "1.5abc".
    if (used != value.size())
...
// One key = value pair lands in its Config field; unknown keys are errors so
// a typo in a .cfg can never silently run with defaults.
static void apply(Config& cfg, const std::string& key, const std::string& value,
    ...
    // Short cfg keys map onto the descriptive GridSpec fields (PLAN §1b).
